...
for key in deployment_output:
    if "Fog" in key:
        ...
        if vm_ip not in copy_done_vm:
            os.system("scp -i {0} {1} dreamlab@{2}:~".format(key_path, copy_file, vm_ip))
...
server_start_command = "sudo docker exec -i {0} java -Xms80m -Xmx800m -cp /edgefs/cli/target/edgefilesystem-0.1-jar-with-dependencies.jar com.dreamlab.edgefs.controlplane.FogServer {1} {2} {3} {4} {5} 0"
